...
| **Type Safety** | `types-inference.md`, `types-custom-types.md`, `types-validators.md` | `$inferSelect`/`$inferInsert`, `InferSelectModel`/`InferInsertModel`, `$type<T>()`, `$defaultFn`, `customType`, branded types, deriving Zod/Valibot/Typebox/Arktype schemas (`drizzle-orm/zod` on v1 vs legacy `drizzle-zod`) |
